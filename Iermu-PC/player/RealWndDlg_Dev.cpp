...
void CRealWndDlg_Dev::OnTimer(UINT_PTR nIDEvent)
{
	...
	case TIMER_ID_HIDE_TEXT:
		...
		player_textout(m_main_dlg->m_hplayer, 0, 0, 0, NULL);
		m_strTxt[0] = '\0';
		break;
	}
}
...
{
	player_textout(m_main_dlg->m_hplayer, 20, 20, RGB(0, 255, 0), m_strTxt);
	SetTimer(TIMER_ID_HIDE_TEXT, time);
}

void CRealWndDlg_Dev::OnSnapshot()
{
	if (!m_main_dlg->m_hplayer) return;

	char ctime[40] = { 0 };
	wchar_t wtime[48] = { 0 };
	GetCurTimeName(ctime, wtime, "snapshot", "jpg");
	player_snapshot(m_main_dlg->m_hplayer, ctime, 0, 0, 1000);
...
void CRealWndDlg_Dev::OnVolume()
{
	...
	player_setparam(m_main_dlg->m_hplayer, PARAM_AUDIO_VOLUME, (void*)&volume);
}
...
void CRealWndDlg_Dev::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (m_main_dlg)
	{
		player_leftbtndown(m_main_dlg->m_hplayer, point.x, point.y);
	}
...
void CRealWndDlg_Dev::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (m_main_dlg)
	{
		player_leftbtnup(m_main_dlg->m_hplayer);
	}
	SetMsgHandled(false);
}
...
void CRealWndDlg_Dev::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_main_dlg)
	{
		player_mousemove(m_main_dlg->m_hplayer, point.x, point.y);
	}
	SetMsgHandled(false);
}
...
BOOL CRealWndDlg_Dev::OnMouseWheel(UINT nFlags, short zDelta, CPoint point)
{
	if (m_main_dlg)
	{
		player_mousewheel(m_main_dlg->m_hplayer, zDelta);
	}
	SetMsgHandled(false);
	return TRUE;
}
