...
CMagnetFrame::AtatchInfo CMagnetFrame::CalcAttachMode(HWND hSubWnd,CRect rcSub)
{
	...
	else if (IsNeighbour(rcMain.CenterPoint().y, rcSub.CenterPoint().y))
	{
		ai.am = AM_CENTER;
		int nDist = CalcDistance(rcMain.CenterPoint().x, rcSub.CenterPoint().x);
		if (nDist == 0) ai.aa = AA_CENTER;
		else if (0 == (nDist = CalcDistance(rcMain.bottom, rcSub.bottom)))
			ai.aa = AA_BOTTOM;
		else
			ai.aa = AA_NULL, ai.nDist = nDist;
	}
	...
	case AM_CENTER:
		{
			...
			{
			case AA_CENTER:
				x = rcMain.left + (rcMain.Width() - rcSub.Width()) / 2;
				y = rcMain.top + (rcMain.Height() - rcSub.Height()) / 2;
				break;
			case AA_BOTTOM:
				x = rcMain.left + (rcMain.Width() - rcSub.Width()) / 2;
			...
			::MoveWindow(hSubWnd, x, y, rcSub.Width(), rcSub.Height(), TRUE);
		}
		break;
    case AM_RIGHT:
        {
